...
task deploy => {
    ...
    update => sub {
        ...
        remote {
            run "export PATH=$perl_dir/bin:\$PATH && cd $deploy_to && git checkout . && git fetch origin && git checkout -q $branch && git submodule update --init && carton install --deployment --cached";
        } $host;
    },
};
